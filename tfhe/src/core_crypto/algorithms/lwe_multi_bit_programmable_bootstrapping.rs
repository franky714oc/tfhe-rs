    ...
    FourierPolyCont: ContainerMut<Element = c64>,
{
    ...
    let multi_bit_fourier_ggsw = unsafe { as_mut_uninit(fourier_ggsw_buffer.as_mut_view().data()) };
...
mod test {
    ...
    fn std_multi_bit_pbs(grouping_factor: LweBskGroupingFactor, thread_count: ThreadCount) {
        // DISCLAIMER: these toy example parameters are not guaranteed to be secure or yield correct
        // computations
        // Define parameters for LweBootstrapKey creation
        let mut input_lwe_dimension = LweDimension(742);
    ...
    #[test]
    fn std_multi_bit_pbs_test_factor_2_thread_5() {
        std_multi_bit_pbs(LweBskGroupingFactor(2), ThreadCount(5));
    }

    #[test]
    fn std_multi_bit_pbs_test_factor_3_thread_12() {
        std_multi_bit_pbs(LweBskGroupingFactor(3), ThreadCount(12));
    }
}
